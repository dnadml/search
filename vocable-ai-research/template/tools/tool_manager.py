...
class ToolManager:
    ...
    def __init__(self, prompt, manual_tool_names, send, is_intro_text):
        self.prompt = prompt
...
async def main():
    ...
    prompt = "AI Trends in US"
    # {'Web Search': SerpGoogleSearchTool(), 'Wikipedia Search': WikipediaSearchTool(),
    #  'Youtube Search': YoutubeSearchTool(), 'Recent Tweets': GetRecentTweetsTool(),
    #  'Full Archive Tweets': GetFullArchiveTweetsTool()}
    tools = ["Wikipedia Search", "Youtube Search", "Web Search"]
    is_intro_text = True
